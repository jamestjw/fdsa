...
module FiniteMap = struct
  ...
  module Make (K : Ord) (V : Value) = struct
    ...
    let size = Tree.size
    let find tree k = Tree.find_cmp ~compare:cmp_key_and_kv tree k |> Option.map drop_key

    let range tree lower upper =
      Tree.range_cmp ~compare:cmp_key_and_kv tree lower upper |> List.map drop_key
    ;;
